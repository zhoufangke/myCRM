...
//实现接口ActionListener
public class gn_genjin implements ActionListener {
...
//	JScrollPane jscrollPane;
	...
	public  gn_genjin(int sid) throws ClassNotFoundException, SQLException {
...
//		jta.append("\n");
		...
		int id=sid;
		// 取得数据库的表的各行数据
		Vector rowData = gengjin_con.getRows(id);
		// 取得数据库的表的表头数据
		Vector columnNames = gengjin_con.getHead(id);
...
//		gn_genjin jiemian=new gn_genjin(1);
//	}
			...
			jb8.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {	
					...
					try {
						...
						preparedStatement = conn.prepareStatement("delete from gengjin where id='"+ sid +"'");
						preparedStatement.executeUpdate();
						
						// 将value数组中的数据依次存放到aa表中
						for(int i = 0; i < row; i++){
							preparedStatement = conn.prepareStatement("insert into gengjin values('" + Integer.parseInt(value[i][0]) + "','" + value[i][1] + "','" + value[i][2] + "','" + value[i][3] +  "')");
							preparedStatement.executeUpdate();
						}
