...
//实现接口ActionListener
public class gn_lianxi implements ActionListener {
...
//	JScrollPane jscrollPane;
	...
	public  gn_lianxi(int sid) throws ClassNotFoundException, SQLException {
...
//		jta.append("\n");
		...
		int id=sid;
		// 取得数据库的表的各行数据
		Vector rowData = lianxi_con.getRows(id);
		// 取得数据库的表的表头数据
		Vector columnNames = lianxi_con.getHead(id);
...
//		gn_lianxi jiemian=new gn_lianxi(1);
//	}
			...
			jb8.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {	
					...
					try {
						...
						preparedStatement = conn.prepareStatement("delete from lianxi where kid='"+ sid +"'");
						preparedStatement.executeUpdate();
						
						// 将value数组中的数据依次存放到aa表中
						for(int i = 0; i < row; i++){
							preparedStatement = conn.prepareStatement("insert into lianxi values('" + Integer.parseInt(value[i][0]) + "','" + Integer.parseInt(value[i][1]) + "','" + value[i][2] + "','" + value[i][3] + "','" + value[i][4] + "','" + value[i][5] + "','" + value[i][6] + "','" + value[i][7] + "','" + value[i][8] +  "')");
							preparedStatement.executeUpdate();
						}
